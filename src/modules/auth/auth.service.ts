...
export const authService = {
  ...
  async forgotPassword(email: string) {
    ...
    await prisma.passwordResetToken.create({ data: { userId: user.id, tokenHash: otpHash, expiresAt } });
  ...
  async verifyOtp(email: string, otp: string) {
    ...
    await prisma.passwordResetToken.update({ where: { id: record.id }, data: { tokenHash: resetHash, expiresAt: new Date(Date.now() + 10 * 60 * 1000) } });
    return { resetToken };
  },
  ...
  async completeOnboarding(userId: string, dto: OnboardingDto) {
    const dailyKcalGoal = this._calculateKcal(dto.currentWeight, dto.activityLevel, dto.goalType, dto.gender);
  ...
  return isNaN(date.getTime()) ? undefined : date;
    ...
    const profile = await prisma.userProfile.upsert({
      where: { userId },
      update: {
        goalType: dto.goalType as any, currentWeight: dto.currentWeight, targetWeight: dto.targetWeight,
        height: dto.height, waistCircumference: dto.waistCircumference, activityLevel: dto.activityLevel as any,
        dietType: dto.dietType as any, allergies: dto.allergies, mealsPerDay: dto.mealsPerDay, dailyKcalGoal,
        gender: dto.gender, dateOfBirth, medicalConditions: dto.medicalConditions ?? [],
        medications: dto.medications, sleepHours: dto.sleepHours, focusTime: dto.focusTime, foodTriggers: dto.foodTriggers,
      },
      create: {
        userId, goalType: dto.goalType as any, startWeight: dto.currentWeight,
        currentWeight: dto.currentWeight, targetWeight: dto.targetWeight, height: dto.height,
  ...
  async getMe(userId: string) {
    return prisma.user.findUnique({
      where: { id: userId },
      select: { id: true, email: true, name: true, role: true, isEmailVerified: true, avatarUrl: true, createdAt: true, updatedAt: true, profile: true, subscription: true },
    });
  },
